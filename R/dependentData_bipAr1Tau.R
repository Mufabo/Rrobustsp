...
#'
#' @export
bip_ar1_tau <- function(x, N, phi_grid, fine_grid, kap2) {
  # AR(0): residual scale equals observation scale
  a_scale_final <- tau_scale(x) # ok
  ...
  for (mm in 1:length(phi_grid)) {
    ...
    a_bip_sc[mm] <- tau_scale(a[2:N])

    # tau-scale of residuals for AR
    a_sc[mm] <- tau_scale(a2[2:N])
  } # for mm in 1:length(phi_grid)
  ...
  if(temp2 < temp) a_scale_final <- c(a_scale_final, tau_scale(a[2:N])) else a_scale_final <- c(a_scale_final, tau_scale(a2[2:N]))
