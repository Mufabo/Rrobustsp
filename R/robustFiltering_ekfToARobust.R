#' ekf_toa
#'
#' Robust EKF for tracking with time-of-arrival (:= ToA) estimates
...
#' @export
ekf_toa_robust <- function(r_ges, theta_init, BS, parameter = NULL){
  if(is.null(parameter)){
    ...
    parameter <- list('dim' = ncol(BS),
                      'var.est' = 1,
                      'max.iters' = 25,
                      'break.cond' = 1e-4,
                      'c1' = 1.5,
                      'c2' = 3)
    parameter$x1 <- fzero(function(x1)parameter$c1-x1*tanh(0.5*x1*(parameter$c2-parameter$c1)), x = 0)$x
  ...
  for(kk in 2:N){
    ...
    Psi <- Matrix::bdiag(matrix(P_min[,,kk], 4, 4), R)

    tryCatch(C <- chol(Psi)
             , finally = Psi + 0.1 * diag(1, M+4, M+4))

    S <- inv(t(C)) %*% rbind(diag(1, 4, 4), H)
    rk <- inv(t(C)) %*% c(th_hat_min[,kk],
                              r_ges[,kk] - h_min + H %*% th_hat_min[,kk])

    th_hat[,kk] <- m_param_est(rk
                               , S
                               , ginv(S) %*% rk
                               , parameter)$th2
